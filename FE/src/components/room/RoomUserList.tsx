...
import { UserList } from "../../pages/Room";
import { ROOM_SEAT_STATE_MAP } from "../../constants/room/roomSeatStateMap";

interface RoomUserListProps {
  userList: UserList;
  setUserList: (value: ((prev: UserList) => UserList) | UserList) => void;
}

export const RoomUserList = ({ userList, setUserList }: RoomUserListProps) => {
  const onToggleClose = (loc: number) => {
    if (typeof userList[loc] === "string") return;
    setUserList((prev) => ({
      ...prev,
      [loc]: !prev[loc],
    }));
  ...
  return (
    <div className="3xl:w-[1225px] w-[980px] 3xl:h-[700px] h-[560px] 3xl:text-[56px] text-[44.8px] font-bold bg-transparent">
      <div className="flex flex-wrap justify-end">
        {userList.map((userSeatState, index) => (
          <div
            className={`3xl:w-[275px] w-[220px] 3xl:h-[337.5px] h-[270px] 3xl:border-[10px] border-[8px] ${
              ...
              typeof userSeatState !== "string" && "cursor-pointer"
            } bg-gray-900 3xl:mb-[25px] mb-[20px] 3xl:mx-[12.5px] mx-[10px]`}
            key={index}
            onClick={() => onToggleClose(index)}
          >
            {typeof userSeatState === "string" && <RoomUserListItem nickName={userSeatState} key={index} />}

            {userSeatState === ROOM_SEAT_STATE_MAP.CLOSE_SEAT && (
